...
class AliceObject(object):
    ...
    def GetObjectInformation(self):
        # Return the objects in current scene
        ...
        if (self.client.get_state() == actionlib.GoalStatus.SUCCEEDED):
            ...
            abs_roi = []
            for roi in rois:
                # Measure and  skip small abberations
                roi_surface = abs(roi.right - roi.left) * abs(roi.top - roi.bottom)
                abs_roi.append(roi_surface)
                if(roi_surface <= 3350):
                    continue
                ...
                all_objects.append((scaled_image, resized_depth_image, roi.x, roi.y, roi.z, roi.z_min, roi.z_max))
            print('Abs value of ROI: ', abs_roi)
